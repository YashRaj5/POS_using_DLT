...
dbutils.widgets.dropdown("mode", "prod", ["prod", "dev"])
...
# MAGIC %md
# MAGIC This notebook is typically run to simulate a new stream of POS data. To ensure data from prior runs are not used in downstream calculations, you should reset the database and DLT engine environment between runs of this notebook:
# MAGIC
...
# MAGIC %md
# MAGIC # Step 1: Assemble Invertory CHnage Reocrds
...
# inventory change record data files (one form each store)
...
inventory_change = (
 ...
 # print a single transaction record to illustrate data structre
eval(inventory_change[0])

# COMMAND ----------

# MAGIC %md
# MAGIC # Step 2: Assemble Invertory Inventory Snapshots
...
# MAGIC %md
# MAGIC To coordinate the transmission of change event data with periodic snapshots, the unique dates and times for which snapshots were taken within a given store location are extracted to a list. This list will be used in the section of code that follows:
# MAGIC
...
# MAGIC %md
# MAGIC # Step 3: Transmit Store Data to the cloud
...
# MAGIC
# MAGIC NOTE Please note that the Azure IOT Hub enforces a limit on the number of events which can be transmitted to it on a per second basis. If this limit is exceeded, you may experience 429 errors from the IOT Hub client below.
# MAGIC
